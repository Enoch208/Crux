...
from crux.repair.candidates import BASELINE_OVERRIDES, V3_OVERRIDES, V4_OVERRIDES
...
OUTPUT_PATH = Path("evidence-dev/qualification_v4.jsonl")
SPEC_PATH = Path("evidence-dev/candidate_v4.json")
RUN_ID = "dev-qualify-5"
SELECTION_SEEDS = tuple(range(101, 133)) + tuple(range(301, 333)) + tuple(range(401, 433))
...
def main() -> int:
    assert_heldout_uncontaminated(HELDOUT_SEEDS, SELECTION_SEEDS)
    ...
    SPEC_PATH.write_text(
        base.with_overrides(dict(V4_OVERRIDES)).model_dump_json(indent=2), encoding="utf-8"
    )
    print(f"controller spec written: {SPEC_PATH}", flush=True)
    print("\n=== virgin qualification (seeds 501-532, never touched before) ===")
    ...
    for left, right in (
        ("baseline-v1", "candidate-v4"),
        ("candidate-v3", "candidate-v4"),
        ("baseline-v1", "candidate-v3"),
    ):
