...
from crux.learning.bc import assert_rocm, load_bc_predictor, make_predictor
from crux.learning.trace import ActionLimits
...
OUTPUT_PATH = Path("evidence-dev/qualification_bc.jsonl")
CHECKPOINT_PATH = Path("evidence-dev/bc_policy.pt")
RUN_ID = "dev-qualify-bc"
TRAINING_SEEDS = tuple(range(101, 133)) + tuple(range(301, 333)) + tuple(range(401, 433))
OTHER_USED_SEEDS = (
    tuple(range(501, 533))
    + tuple(range(601, 633))
    + tuple(range(701, 733))
...
HELDOUT_SEEDS = tuple(range(1001, 1033))
MAX_CHUNKS = 900
BUDGET_STEPS = 20000
MAX_STEP_M = 0.030
...
CONFIDENCE = 0.95
BC_VERSION = "bc-v1"
ARMS: tuple[str, ...] = ("baseline-v1", "candidate-v4", BC_VERSION)


def action_limits(config: TaskConfig) -> ActionLimits:
    layout = config.layout
    return ActionLimits(
        max_step_m=MAX_STEP_M,
        min_force_n=config.control.close_force_n * 1.5,
        max_force_n=config.control.open_force_n,
        workspace_low=(layout.cable_base[0] - 0.45, layout.cable_base[1] - 0.45, 0.008),
        workspace_high=(layout.cable_base[0] + 0.45, layout.cable_base[1] + 0.45, 0.40),
    )


def main() -> int:
    assert_heldout_uncontaminated(HELDOUT_SEEDS, TRAINING_SEEDS + OTHER_USED_SEEDS)
    device = assert_rocm()
    model, standardiser = load_bc_predictor(CHECKPOINT_PATH, device)
    predictor = make_predictor(model, standardiser, device)

    config = load_task_config()
    limits = action_limits(config)
    n_envs = len(ARMS) * len(HELDOUT_SEEDS)
    scene = stage(
    ...
    for version in ARMS:
        for seed in HELDOUT_SEEDS:
            ...
            assignments.append((version, seed, params))
    ...
    for env, (version, _seed, params) in enumerate(assignments):
        policy: ControlPolicy
        if version == BC_VERSION:
            policy = LearnedEpisodePolicy(
                config=config,
                predictor=predictor,
                limits=limits,
                budget_steps=BUDGET_STEPS,
                timestep_s=scene.timestep_s,
            )
        else:
            overrides = BASELINE_OVERRIDES if version == "baseline-v1" else V4_OVERRIDES
    ...
    for env, (version, seed, params) in enumerate(assignments):
    ...
    by_version = {v: [r for r in records if r.controller_version == v] for v in ARMS}

    print("\n=== learned-policy qualification: virgin seeds 1001-1032 ===")
    for version in ARMS:
    ...
    for left, right in (("baseline-v1", BC_VERSION), ("candidate-v4", BC_VERSION)):
