...
OUTPUT_PATH = Path("evidence-dev/qualification_scale.jsonl")
RUN_ID = "dev-qualify-scale"
PRIOR_SEEDS = (
    tuple(range(101, 133))
    + tuple(range(301, 333))
    + tuple(range(401, 433))
...
HELDOUT_SEEDS = tuple(range(801, 929))
MAX_CHUNKS = 900
...
def main() -> int:
    assert_heldout_uncontaminated(HELDOUT_SEEDS, PRIOR_SEEDS)
    ...
    print(f"\n=== scaled qualification: {len(HELDOUT_SEEDS)} virgin seeds per arm (801-928) ===")
    ...
    if by_version["baseline-v1"] and by_version["candidate-v4"]:
        depth = compare_stage_reached(
            by_version["baseline-v1"], by_version["candidate-v4"], ENDPOINT
        )
        print(
            f"\n  baseline-v1 vs candidate-v4, reached {ENDPOINT}: "
            ...
            f"McNemar p = {depth.mcnemar_p_value:.2e}"
        )
