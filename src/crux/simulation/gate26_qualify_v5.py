...
HELDOUT_SEEDS = tuple(range(701, 733))
MAX_CHUNKS = 900
...
def main() -> int:
    ...
    SPEC_PATH.write_text(
        base.with_overrides(dict(V5_OVERRIDES)).model_dump_json(indent=2), encoding="utf-8"
    )
    print(f"controller spec written: {SPEC_PATH}", flush=True)
    print("\n=== virgin qualification (seeds 701-732, never touched before) ===")
    ...
    for left, right in (
        ("baseline-v1", "candidate-v5"),
        ("candidate-v4", "candidate-v5"),
